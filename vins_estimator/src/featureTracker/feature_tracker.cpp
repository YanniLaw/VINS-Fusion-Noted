...
void FeatureTracker::setMask()
{
    ...
    // 按跟踪时间从大到小进行排序，即历史稳定的点在前面，新出现的点在后面。优先保留老特征点的位置
...
map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> FeatureTracker::trackImage(double _cur_time, const cv::Mat &_img, const cv::Mat &_img1)
{
    ...
    */
    ...
    {
        TicToc t_o;
        vector<uchar> status;   // 是否找到对应点 1: 成功跟踪 0: 跟踪失败
        vector<float> err;      // 每个点的跟踪误差（如残差或SSD值）
        /** cv::calcOpticalFlowPyrLK()是 OpenCV 中最常用、最经典的稀疏光流（Sparse Optical Flow）跟踪算法函数，
         * 常与 cv::goodFeaturesToTrack() 一起使用（前者检测角点，后者追踪角点）。 
         * 用于在两帧图像之间跟踪特征点的运动，即估计每个特征点在图像中的光流位移。
         * 它实现了 Lucas–Kanade 光流算法 的金字塔版本（Pyramidal LK）[引入金字塔是由于普通LK光流只能处理小位移]
         * void cv::calcOpticalFlowPyrLK(
                InputArray prevImg,     // 前一帧灰度图像
                InputArray nextImg,     // 当前帧灰度图像
                InputArray prevPts,     // 前一帧中的特征点坐标
                OutputArray nextPts,    // 输出：在当前帧中对应的点位置（估计结果）
                OutputArray status,     // 输出：与每个点对应的标志位（1=成功跟踪，0=跟踪失败）
                OutputArray err,        // 输出：每个点的跟踪误差（如残差或SSD值）
                Size winSize = Size(21, 21),    // 每个金字塔层上搜索的窗口大小（通常 21×21 或 15×15）
                int maxLevel = 3,       // 图像金字塔层数（0 表示不使用金字塔，这个时候也就是普通LK光流了）
        ...
        *       你需要根据 status 向量来手动删除那些跟丢的点（status[i] == 0 的点
        ...
        if(FLOW_BACK)
        {
            ...
            cv::calcOpticalFlowPyrLK(cur_img, prev_img, cur_pts, reverse_pts, reverse_status, err, cv::Size(21, 21), 1, // 这里金字塔层数只有1层
            cv::TermCriteria(cv::TermCriteria::COUNT+cv::TermCriteria::EPS, 30, 0.01), cv::OPTFLOW_USE_INITIAL_FLOW);
            //cv::calcOpticalFlowPyrLK(cur_img, prev_img, cur_pts, reverse_pts, reverse_status, err, cv::Size(21, 21), 3); 
            for(size_t i = 0; i < status.size(); i++)
            {
                // 前向光流和反向光流都要跟踪成功 且 像素闭环误差在给定阈值之内
    ...
    if (1)
    {
        ...
        TicToc t_m;
        setMask(); // 按照跟踪时间重新筛选并生成新的跟踪点集
        ROS_DEBUG("set mask costs %fms", t_m.toc());

        ROS_DEBUG("detect feature begins");
        TicToc t_t;
        int n_max_cnt = MAX_CNT - static_cast<int>(cur_pts.size());
        if (n_max_cnt > 0) // 检测新的特征点
        {
            if(mask.empty())
            ...
            /**
             * goodFeaturesToTrack 是 OpenCV 的 Shi-Tomasi 角点检测算法，用于检测质量较好的角点
...
/**
 * @brief 对图像特征点进行去畸变，并转换到归一化相机平面坐标系
 * 
 * @param pts 特征点图像坐标
 * @param cam 相机模型
 * @return vector<cv::Point2f> 去畸变后的归一化坐标
 */
vector<cv::Point2f> FeatureTracker::undistortedPts(vector<cv::Point2f> &pts, camodocal::CameraPtr cam)
{
    ...
    for (unsigned int i = 0; i < pts.size(); i++)
    {
        ...
        un_pts.push_back(cv::Point2f(b.x() / b.z(), b.y() / b.z())); // 归一化坐标
    }
    return un_pts;
}
/**
                                            ...
                                            map<int, cv::Point2f> &cur_id_pts, map<int, cv::Point2f> &prev_id_pts)
{
    ...
    cur_id_pts.clear();
    for (unsigned int i = 0; i < ids.size(); i++) // 更新当前帧跟踪到的特征点的关联容器
    {
...
/**
 * @brief 可视化函数，在图像上绘制当前帧和上一帧的特征点跟踪结果（包括双目匹配
 * 
 * @param imLeft 当前帧左目图像
 * @param imRight 当前帧右目图像
 * @param curLeftIds 当前帧左目跟踪到的特征点id
 * @param curLeftPts 当前帧左目特征点坐标
 * @param curRightPts 当前帧右目特征点坐标
 * @param prevLeftPtsMap 上一帧特征点id跟特征点坐标映射关系
 */
                               ...
                               map<int, cv::Point2f> &prevLeftPtsMap)
{
    ...
    if (!imRight.empty() && stereo_cam) // 双目模式进行拼接
        cv::hconcat(imLeft, imRight, imTrack); // 将双目图像水平地（按列）拼接在一起，形成一个更大的新图像 (imTrack是输出的图像)
    else
        imTrack = imLeft.clone();
    cv::cvtColor(imTrack, imTrack, CV_GRAY2RGB); // 颜色转换
    // 绘制左目特征点
    for (size_t j = 0; j < curLeftPts.size(); j++)
    {
        double len = std::min(1.0, 1.0 * track_cnt[j] / 20); // 根据跟踪时间长短来确定特征点的颜色
    ...
    if (!imRight.empty() && stereo_cam)
    {
        for (size_t i = 0; i < curRightPts.size(); i++)
        {
            ...
            rightPt.x += cols; // 将右图特征点花在右边(右目图像是拼接在左目的右边的)
